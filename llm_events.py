...
## 可用物品 ID 列表（只能使用这些）
武器: rusty_knife, baseball_bat, hunting_rifle, fire_axe, crossbow
防具: leather_jacket, riot_shield, military_vest
消耗品: canned_food, bottled_water, bandage, first_aid_kit, mre, stimpack, antidote
材料: scrap_metal, cloth, rope, nails, glass, electronics, gunpowder, herb, leather, plastic
特殊: survivor_journal, radio
...
async def generate_batch(context, season: str, weather: str,
    ...
    if _cache_lock:
        return 0
    if time.time() - _last_generate_time < GENERATE_COOLDOWN:
        return 0
    ...
    try:
        ...
        # 成功后更新时间戳，避免 cooldown 阻挡同一次 refill 的后续批次
        _last_generate_time = time.time()
        return len(events)

    except Exception as e:
        print(f"[LLMEventGen] 生成事件失败: {e}")
        return 0
    finally:
        _cache_lock = False
